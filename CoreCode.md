...
bool WriteEditCSV(Domacnost d_data[], int NumberOfID,string adress)
{
    ...
    for (int i=0; i<NumberOfID; i++)
    {
        for (int j=1; j<=pozition; j++)
        {
            ...
            case 2:
            {
                editcsv<<BoolToStr(d_data[i].pv)<<";";
                break;
            }
            case 3:
            {
                editcsv<<d_data[i].kategorie<<";";
                break;
            }
            case 4:
            {
                editcsv<<d_data[i].castka<<";";
                break;
            }
            case 5:
            {
                editcsv<<d_data[i].datum;
                break;
            }
            }
        }
...
bool CheckMonth(Domacnost d_data[], int Month, int Number, bool Mode)
{
    ...
    else
    {
        ...
        {
            if (Month+1==d_data[i].mesic)return true;
        }
        return false;
    }
}
 ...
 */
void SortMaxToMin(Domacnost d_data[],Memory_sort mem_data[], int EndInt, int StartInt,int sump,int sumv,int AllID)
{
    int NumberOfID=0;
    int HelpCount=0;
    for (int m=StartInt; m<EndInt; m++)
    {
        if (CheckMonth(d_data,m,AllID,false))
        {
 ...
 * @param startint Pocatek intervalu vypisu zaznamu
 */
bool HTML_All(string name,Domacnost d_data[], Memory_sort mem_data[], int suma_p, int suma_v, int startint, int endofint, int AllID)
{
    ...
    {
        if (CheckMonth(d_data,j,AllID,false))
        {
    ...
    bool Menu(Domacnost d_data[], Memory_sort mem_data[], int NumberOfID, int NumberOfMonths,string adress)
    {
        ...
        while (!exit_f)
        {
            ...
            case 1:
            {
                ...
                SortMaxToMin(d_data,mem_data,end_int, start_int, suma_p, suma_v,NumberOfID);
                optionsS = SmallMenu();
                if (optionsS == 1)
                {
                    string name = NameOfFile();
                    if (!HTML_All(name,d_data, mem_data, suma_p, suma_v, start_int, end_int - 1,NumberOfID))
                        error("Chyba pri ukladani do HTML!");
                    else
                        cout << "Soubor " << name << " byl uspesne ulozen do HTML!" <<" Cesta:..\\vystupnidata\\" + name<<endl;
                    break;
            ...
            case 3:
            {
                ...
                {
                    string name = NameOfFile();
                    if (!HTML_All(name,d_data, mem_data, suma_p, suma_v, start_int, end_int,NumberOfID))
                        error("Chyba pri ukladani do HTML!");
                    else
                        cout << "Soubor " << name << " byl uspesne ulozen do HTML!" <<" Cesta:..\\vystupnidata\\" + name<<endl;
                    break;
